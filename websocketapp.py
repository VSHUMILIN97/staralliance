...
async def subscribe_to_redis(path):
    conn = await create_connection((LOCAL_SERVICE_HOST, REDIS_DEFAULT_PORT))
    # Set up a subscribe channel
    channels = Channel('s-*', is_pattern=True)
    await conn.execute_pubsub('subscribe', channels)
    return channels, conn
