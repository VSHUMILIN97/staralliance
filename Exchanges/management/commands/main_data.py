...
def livecoin_subprocess():
    try:
        script_path = os.path.join(os.path.dirname(__file__), '../../../Exchanges/ExchangeAPI/coinmarketAPI.py')
    except FileNotFoundError:
        logging.error(u'CoinmarketcapAPI process could not be reached. Check File path')
        sys.exit(-1)
    ...
    try:
        ...
        logging.error(u'CoinmarketcapAPI process could not be started. Check logs.')
        sys.exit(-1)
    # PID Declaration
