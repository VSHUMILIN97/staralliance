...
logging.basicConfig(format=u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s',
                    level=logging.DEBUG, filename='/var/log/cryptopiper/liquiAPI.log')


def livecoin_subprocess():
    try:
        script_path = os.path.join(os.path.dirname(__file__), '../../../Exchanges/ExchangeAPI/liquiAPI.py')
    except FileNotFoundError:
        logging.error(u'LiquiAPI process could not be reached. Check File path')
        sys.exit(-1)
    ...
    try:
        ...
        logging.error(u'LiquiAPI process could not be started. Check logs.')
        sys.exit(-1)
    # PID Declaration
