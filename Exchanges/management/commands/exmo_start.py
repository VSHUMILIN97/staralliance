...
def poloniex_subprocess():
    try:
        script_path = os.path.join(os.path.dirname(__file__), '../../../Exchanges/ExchangeAPI/ExmoAPI.py')
    except FileNotFoundError:
        logging.error(u'ExmoAPI process could not be reached. Check File path')
        sys.exit(-1)
    ...
    try:
        ...
        logging.error(u'ExmoAPI process could not be started. Check logs.')
        sys.exit(-1)
    # PID Declaration
